...
export const kinematicsLessons: LessonSeed[] = [
  // ───────────────── 1. Position, Displacement & Distance ─────────────────
  {
    ...
    questions: [
      { scope: "CONCEPT_CHECK", kind: "TRUE_FALSE", prompt: "Displacement is a vector — it has a direction.", answer: true, difficulty: 1, explanation: "Yes. Displacement carries a sign (direction); distance does not." },
      ...
      { scope: "CONCEPT_CHECK", kind: "MATCHING", prompt: "Match each symbol to what it represents.", options: { left: ["$x$", "$\\Delta x$", "$x_i$", "$x_f$"], right: ["Position", "Displacement", "Initial position", "Final position"] }, answer: ["Position", "Displacement", "Initial position", "Final position"], explanation: "$x$ is position, $\\Delta x = x_f - x_i$ is displacement, and $x_i$/$x_f$ are the start and end positions." },
      { scope: "CONCEPT_CHECK", kind: "FILL_BLANK", prompt: "Distance is a ___ (size only), while displacement is a ___ (size and direction).", answer: [["scalar"], ["vector"]], explanation: "Distance is a scalar; displacement is a vector." },
      { scope: "CONCEPT_CHECK", kind: "ORDER", prompt: "Order the steps to find an object's displacement.", options: ["Choose an origin and a positive direction", "Record the initial position $x_i$", "Record the final position $x_f$", "Compute $\\Delta x = x_f - x_i$"], answer: [], explanation: "Set up a reference, read off the start and end positions, then subtract." },
      { scope: "CONCEPT_CHECK", kind: "GRAPH", prompt: "Sketch the position–time graph of an object moving at a constant $2\\,\\text{m/s}$ from the origin — a straight line through the origin with slope 2. Drag the points to trace it.", answer: { expr: "2*x", domain: [0, 5], variable: "x", mode: "draw", samples: 7 }, difficulty: 3, hint: "At $t = 0$ the position is 0; at $t = 5$ it is 10.", explanation: "Constant velocity gives a straight line $x = 2t$ — slope 2, through the origin." },
